 ...
 * element is hidden, on both core versions this extension supports. It is brought
 * back by the page TSconfig of the component, which is delivered either by the site
 * set "fgtclb/academic-bite-jobs-list" or, as here, by the entry
 * "Configuration/TCA/Overrides/pages.php" registers for the page field
 * "Page TSconfig". On TYPO3 v12 the latter is the only one of the two that exists -
 * site sets arrived in v13.1 - which is what makes this class the one that matters
 * there.
 *
...
final class NewContentElementWizardRegistrationTest extends AbstractAcademicBiteJobsTestCase
{
    ...
    /**
     * On TYPO3 v12 an element that is configured but not listed in the group's "show"
     * is never added to the wizard: `NewContentElementController` builds the wizard
     * from page TSconfig there and gates every element on that list. The
     * "show := addToList()" line therefore has to travel with the element definition,
     * in the very same file, so that both arrive through one and the same include.
     *
     * Asserted on both core versions on purpose. The list has no reader left on v13,
     * where the wizard is built from TCA (Feature: #102834), but it is still shipped
     * for v12 and a delivery that lost it would be a v12-only defect that no v13 test
     * could see.
     */
    #[Test]
    public function wizardGroupShowsTheContentElementWithTheRegisteredPageTsConfig(): void
    {
        $this->importCSVDataSet(
            __DIR__ . '/Fixtures/NewContentElementWizardRegistration/pagesWithRegisteredPageTsConfig.csv'
        );
        ...
        $this->assertNotContains(
            'academicbitejobs_list',
            $this->shownContentElementTypes(1),
            'The content element is shown on the page without the registered page TSconfig.',
        );
        $this->assertContains(
            'academicbitejobs_list',
            $this->shownContentElementTypes(2),
            'The registered page TSconfig did not add the content element to the wizard group.',
        );
    }
    ...
    /**
     * @return list<string> The content element types the wizard group "academic" lists
     *                      in "show" on the given page.
     */
    private function shownContentElementTypes(int $pageId): array
    {
        $group = BackendUtility::getPagesTSconfig($pageId)['mod.']['wizards.']['newContentElement.']['wizardItems.']['academic.'] ?? [];

        return GeneralUtility::trimExplode(',', (string)($group['show'] ?? ''), true);
    }
}
