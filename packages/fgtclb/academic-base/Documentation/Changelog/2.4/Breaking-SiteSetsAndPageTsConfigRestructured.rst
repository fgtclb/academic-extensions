...
now reachable in three ways, all of them reading that one copy. Note the
directory as well: :file:`Configuration/TsConfig/` was renamed to
...
*   The site set :yaml:`fgtclb/academic-base-ctype-group`, which points its
    :yaml:`pagets` key at the file instead of carrying a copy. Site sets
    arrived in TYPO3 v13.1, so this way exists on v13 and not on v12.
