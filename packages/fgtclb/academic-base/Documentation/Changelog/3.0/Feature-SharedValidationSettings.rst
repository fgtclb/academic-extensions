...
the TCA merge. `EXT:academic_persons` is the first to do so; see its own
changelog for the moved class names. Two edge cases of its loader change on
purpose: a settings file that parses to a scalar is skipped like an empty one
instead of failing the merge, and a numeric YAML key is cast to a string
instead of failing the value object constructors. `EXT:academic_base` now
declares its dependency on Extbase, whose validator classes the normaliser
names.
