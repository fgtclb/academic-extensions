...
class ProfileImageEditorElement extends ProfileEditingElement {
   ...
   *
   * Named `applyState()` rather than `render()`: `render()` belongs to
   * `LitElement` and this element renders nothing - it writes attributes,
   * classes and the `hidden` flag onto markup Fluid produced.
   */
  applyState() {
    var _a;
    const context = this.#context;
