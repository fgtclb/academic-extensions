...
const profileFieldsColumnSelector =
  ".academic-persons-inline-edit__profile-fields-column";
...
export const parseImageRatio = (value: unknown): number | null => {
...
export const configureCropperSelection = (
...
export const getImagePreviews = (root: HTMLElement): HTMLElement[] =>
  Array.from(
...
export const setImagePreviewUrl = (
  preview: HTMLElement,
  url: string,
  alt = "",
  title = "",
...
export const setImageState = (root: HTMLElement, hasImage: boolean): void => {
...
export const createImageEditing = (
    ...
    void nextTick().then((): void => {
      requestAnimationFrame((): void => {
        requestAnimationFrame((): void => {
          ...
          root
            .querySelector<HTMLButtonElement>("[data-ie-open-image-view]")
  ...
  const deleteImage = async (): Promise<void> => {
    ...
    if (image.pending || !image.hasImage || profile === null || endpoint === undefined) {
      return;
    }
    image.pending = true;
    image.error = "";
