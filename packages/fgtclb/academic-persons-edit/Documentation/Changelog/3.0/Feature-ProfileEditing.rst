...
(*Breaking: Profile information years become dates* in
...
Nothing else is bundled. The view is Fluid, driven by five custom elements built
on Lit, which TYPO3 delivers through the import map of ``EXT:core``, and
CKEditor 5 is loaded from the system extension ``rte_ckeditor`` - see
