...
describe("the document editor element", () => {
  ...
  it("writes its own children and opens no shadow root", async () => {
    ...
    // The base class returns a detached node from "createRenderRoot()", so
    // lit-html renders into something that is not this element and can never
    // reach the children below. What is here is what this element put here.
    assert.equal(element.shadowRoot, null);
    assert.notEqual(element.renderRoot, element);
    assert.equal((element.renderRoot as Node).isConnected, false);
