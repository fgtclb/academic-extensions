...
/**
 * Covers the complete multilingual lifecycle of the inline profile image.
 *
 ...
 */
#[Group('not-core-13')]
final class AcademicPersonsEditInlineProfileImageTranslationTest extends AbstractFrontendProfilePluginTestCase
{
    ...
    private function extractImageForm(string $content): array
    {
        $this->assertSame(
            ...
            'The default-language profile has no inline image form.',
        );
    ...
    #[Test]
    public function upgradeWizardSeparatesLegacyLocalizedImageReference(): void
    {
        $this->setUpTestCase();

        $this->assertSame(200, $this->uploadImage()->getStatusCode());
        $this->assertGreaterThan(0, $this->getTranslationUid());

        $subject = $this->get(RepairLocalizedProfileImagesUpgradeWizard::class);
        ...
        $this->assertSame(0, $references[1]['l10n_parent']);
        $this->assertNotSame($references[0]['uid_local'], $references[1]['uid_local']);
        $this->assertSame('custom', $this->getImageLocalizationState($this->getTranslationUid()));
        $this->assertFileReferenceIndexMatches($references);
        $this->assertCount(2, $this->getStoredFiles());
    }

    #[Test]
    public function upgradeWizardRepairsDanglingProfileImageCounter(): void
    {
        $this->setUpTestCase();
