...
partials only, and a change to either has no effect on that page. The same
override also loses every timeline date, because the properties its markup
prints - ``{item.year}``, ``{item.yearStart}``, ``{item.yearEnd}`` - do not
exist any more and Fluid resolves a missing property to an empty string; see
:ref:`breaking-profile-information-years-become-dates` for the replacements and
