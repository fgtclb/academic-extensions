...
final class ProfileTest extends UnitTestCase
{
    ...
    /**
     * `Backend::addCommonFieldsToRow()` writes `_languageUid` back onto the object while it
     * inserts, so an unpersisted object's language is not yet the one its row will carry.
     * `_isNew()` keeps the answer from depending on that.
     */
    #[Test]
