...
final class LegacySettingsMigrator implements LoggerAwareInterface
{
     ...
     * record type and relation field. One that does not is the eighth type:
     * there is no profile relation for it, so it is reported and not created.
     *
     * @param array<string, mixed> $settings
     * @param list<string> $notes
     * @return array<string, mixed>
    ...
    private function migrateProfileInformationTypes(array $settings, array &$notes): array
    {
        ...
        foreach ($types as $identifier => $type) {
            ...
            foreach (['label', 'type', 'fieldName'] as $option) {
                $value = $type[$option] ?? null;
                if (is_string($value) && trim($value) !== '') {
                    $sections[$identifier][$option] = trim($value);
                }
            }
        }
        if (is_array($sections)) {
