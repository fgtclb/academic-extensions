 ...
 *
 * The header of the content element renders once: by default the content element layout of
 * `lib.contentElement`, which `PLUGIN_TYPE_CONTENT_ELEMENT` wires up, renders it and the
 * template does not. A site whose layout renders no header switches
 * `renderContentElementHeader` on, and the template then renders the
 * `EXT:fluid_styled_content` `Header/All` partial itself. On TYPO3 v14 that partial renders
 * the header through the `record` view variable, which the switched on cases guard.
 */
final class AcademicContacts4PagesListPluginTest extends AbstractAcademicContacts4PagesTestCase
{
    ...
    private const HEADER = 'Your contacts';
    private const SUBHEADER = 'We are glad to help';
    private const RENDER_HEADER_CONSTANTS = 'EXT:academic_contacts4pages/Tests/Functional/Plugins/Fixtures/TypoScript/Constants/RenderContentElementHeader.typoscript';
    private const HEADER_PARTIAL_OVERRIDE_SETUP = 'EXT:academic_contacts4pages/Tests/Functional/Plugins/Fixtures/TypoScript/Setup/HeaderPartialOverride.typoscript';
    private const LAYOUT_WITHOUT_HEADER_SETUP = 'EXT:academic_contacts4pages/Tests/Functional/Plugins/Fixtures/TypoScript/Setup/LayoutWithoutHeader.typoscript';
     ...
     *
     * It replaces the path of key `10`, which is where the shipped `Contacts/Item.html` of
     * this extension lives, so it also pins that Extbase keeps resolving that partial: it
     * adds the extension's own partial path whenever the configured ones lack it.
     */
    #[Test]
    ...
    /**
     * A site package that renders the header of this element in its list template, through
     * the header partial of `EXT:fluid_styled_content`, needs `record` on TYPO3 v14, where
     * that partial renders the header through it. The fixture moves the header out of the
     * layout of the element, as such a site package does, so it renders exactly once. The
     * uid is printed so the variable is asserted on v13 too, whose partial reads `data`.
     */
    #[Test]
    public function aListTemplateOverrideCanRenderTheCoreHeaderPartial(): void
    {
        $this->setUpTestCase(
            'contactsListPage',
            [],
            ['EXT:academic_contacts4pages/Tests/Functional/Plugins/Fixtures/TypoScript/Setup/HeaderRenderingListTemplate.typoscript'],
        );
        $this->setContentElementHeader(1, 0);

        $document = new \DOMDocument();
        $document->loadHTML($this->renderHomePage(), LIBXML_NOERROR);
        $xpath = new \DOMXPath($document);
        $headers = $this->nodes($xpath, "//*[self::h1 or self::h2 or self::h3][normalize-space() = 'Your contacts']");
        $this->assertSame(1, $headers->length, 'The header renders exactly once.');
        $this->assertSame(
            1,
            $this->countNodes($xpath, "//div[@class = 'list-override']//*[self::h1 or self::h2 or self::h3][normalize-space() = 'Your contacts']"),
            'The header renders in the list template.',
        );
        $this->assertSame(
            'RECORD-UID[1]',
    ...
    {
        $this->setUpTestCase('contactsListPage', [self::RENDER_HEADER_CONSTANTS], [self::LAYOUT_WITHOUT_HEADER_SETUP, self::HEADER_PARTIAL_OVERRIDE_SETUP]);
