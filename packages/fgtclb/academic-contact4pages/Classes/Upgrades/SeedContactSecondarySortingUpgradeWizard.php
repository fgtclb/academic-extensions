...
/*
 * This file is part of the "academic_contacts4pages" Extension for TYPO3 CMS.
 *
...
/**
 * Seeds `contract_sorting` and `role_sorting`, the sort columns the contract and
 * the contacts role relation gained in 3.0.
 *
 * A page contact is an inline child of three records: of its page, of the contract
 * it belongs to, and of its contacts role. All three relations wrote the shared
 * `sorting` column until now, so saving a contract or a contacts role renumbered
 * its contacts across the pages that own them. Both other relations order by a
 * column of their own from 3.0 on, and every contact that existed before the
 * update carries `0` in them - which would leave those forms showing their
 * contacts in whatever order the database happens to return.
 *
 * This wizard appends every contact that has no rank yet to the list of its
 * contract and of its contacts role, in the current (`sorting`, `uid`) order - the
 * order those forms show today, with their ties settled by uid. On an installation
 * that has just updated nothing has a rank, so that numbers everything 1..n in
 * exactly that order.
 *
 * **Appending rather than renumbering** is what makes it safe to run again: the
 * order in those forms is an arrangement from this version on, and a second run
 ...
 * {@see \FGTCLB\AcademicContacts4pages\Hook\ContactSortingHook} gives every
 * contact that joins a contract or a contacts role afterwards a rank of its own.
 *
 * **Every row carrying a contract or a contacts role is numbered**, deleted ones
 * and workspace versions included. The columns are a rank within their parent, and
 * ordering only ever compares rows: leaving a row at `0` would put it above every
 * numbered sibling the moment it is restored or published. Numbering by the parent
 * uid the row carries is the same grouping `RelationHandler` itself uses when it
 * writes the column, so the relative order of any set of rows it lists - a live
 * list, a workspace list - is the order this wizard seeds.
 */
#[UpgradeWizard('academicContact4pages_seedContactSecondarySorting')]
final readonly class SeedContactSecondarySortingUpgradeWizard implements UpgradeWizardInterface
{
    ...
    {
        return 'Seed the contact sort order of contracts and contacts roles';
    }

    public function getDescription(): string
    {
        return 'Page contacts are inline children of their page, of their contract and of their'
            . ' contacts role, and all three relations wrote the same sort column until now, so saving'
            . ' a contract or a contacts role could rearrange the contacts of a page. The contract and'
            . ' the contacts role relation have a sort column of their own from 3.0 on. This wizard'
            . ' fills them with the order those forms show today.';
    }
    ...
    /**
     * Every row that carries the given parent, grouped by it and in the order the
     * form of that parent shows today. The ordering is part of the result, so it is
     * the statement that has to produce it - a re-sort in PHP would have to reproduce
     * the collation.
     *
