...
/**
 * Keeps `contract_sorting` and `role_sorting` - the sort columns of the contract and
 * the contacts role relation - filled, for every contact that joins one of them
 * outside that record's own form.
 *
 * `RelationHandler::writeForeignField()` numbers the children of the record being
 * saved, so it fills a column only when the **contract** or the **contacts role** is
 * saved. The usual way a contact gets either is the other one: the `contract` and
 * `role` selects of the contact form, which editors reach through the inline list of
 * the page. Without this hook such a contact keeps `0` and sorts above every
 * numbered sibling in that list - and two of them tie, which is exactly the
 * arbitrary order the sort columns exist to remove.
 *
 * The rule is the one an inline list follows anyway: **a contact that joins a
 * contract or a contacts role is appended to the end of that record's list.** It
 * applies when the contact is created with one, when it changes (the rank it had
 * under the old one means nothing under the new one), and when it is copied or
 * localized. A contact that already has a rank under the record it is being saved
 * with keeps it, so an arrangement made in that form survives every later save of
 * the contact. Clearing the relation resets its column to `0`.
 *
 ...
 *   record, in the order the data map is processed, so a second new contact of the
 *   same contract sees the first one already stored and lands behind it.
 ...
 */
final class ContactSortingHook
{
     ...
     * The page relation is not among them: it owns the shared `sorting` column,
