 ...
 * `plugin.tx_<name>_<plugin>`, and up to TYPO3 v13 the core builds that path from the name
 ...
 *
 * Without a name, the core picks one itself, and not the same one on every version: TYPO3
 * v13 takes the name of the plugin request, TYPO3 v14 the extension key of a full
 * `LLL:EXT:` reference first. A help text of the profile editor that points into the file
 * of `academic_persons` therefore read the overrides of the editor on v13 and those of
 * `academic_persons` on v14.
 *
 * The functional label tests of each extension prove every kind of call, but on TYPO3 v13
 * only as the first translation of its file in a request, since v13 keeps the overrides one
 * translation has read for the ones after it, and on TYPO3 v14 not the name at all: v14
 * strips the underscores and falls back to the name of the request. This test holds the
 * templates, and the names handed to `translate()` directly in PHP. A name in an argument
 * list that is built first and then spread is not seen; each of those is the first
 * translation of its file, which the functional label tests prove.
 */
final class TranslationExtensionNameTest extends TestCase
{
    ...
    public function noTemplatePassesAnExtensionNameWithAnUnderscore(string $directory): void
    {
        ...
        $this->assertSame(
            ...
            . ' or TYPO3 v13 reads the _LOCAL_LANG overrides of these calls from the wrong path.',
        );
    }
    ...
    public function everyTranslationNamesItsExtension(string $directory): void
    {
        ...
        $this->assertSame(
            ...
            . ' picks one, and TYPO3 v13 and v14 pick different ones for a full LLL:EXT: reference.',
        );
    }
    ...
    public function noTranslationInPhpPassesAnExtensionNameWithAnUnderscore(string $directory): void
    {
        ...
        $this->assertSame(
            ...
            $found,
            'Pass the extension name in UpperCamelCase - "AcademicPartners", not "academic_partners" -'
            . ' or TYPO3 v13 reads the _LOCAL_LANG overrides of these translations from the wrong path.',
        );
    }
