...
- [x] 2.2 Run group 1 and the existing content element tests green on v13,
  then on v14.
- [ ] 2.3 Cover what that check was for, by test rather than by hand. The
  ...
  spacing and anchor are asserted here by the functional test on the
  EXT:fluid_styled_content layout. The interaction follows in the next commit.
...
- [x] 5.3 Commit as `[BUGFIX] ACE-702: Honour the study plan appearance`
  in TYPO3 Core format, with the node coverage as a second commit.
