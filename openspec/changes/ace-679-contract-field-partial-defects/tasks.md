...
- [x] 1.2 Add tests to `AcademicPersonsCardPluginTest` asserting the label
...
- [x] 1.3 Add a selected-contracts test with the same phone number, so the
...
- [x] 2.2 Add `contracts.organisationalUnit` to `locallang.xlf` and
  ...
  assertion passes.
- [x] 2.3 Build the `tel:` target with `f:replace` as `Contact.html` does and
  verify the tests of 1.2 and 1.3 pass.
- [x] 2.4 Break each change on purpose once more (drop the branch, restore
  the raw href) and watch the matching assertion go red; restore.

## 3. Phone link prefix

- [x] 3.1 Add tests first: a card, a selected-contracts and a detail test with
...
- [x] 3.2 Declare `plugin.tx_academicpersons.phoneNumbers.telPrefix` (string,
  ...
  template.
- [x] 3.3 Verify whether the contacts processor of `academic_contacts4pages`
  renders the item with plugin settings that carry the prefix; map it there
  too, or name the gap in the pull request.
- [x] 3.4 Prepend the prefix before removing spaces in `Field.html` and in
...
- [x] 4.1 Check `Documentation/` for a description of the `showFields` items
  and correct it if it lists the unit as unsupported. Verify with
  `checkRstRenderingAll`.
- [x] 4.2 Document the prefix setting in the configuration chapter of
  `Documentation/`: it is unconditional, and meant for installations that
  store extensions only.
- [x] 4.3 Add `Documentation/Changelog/3.0/Feature-PhoneLinkPrefix.rst` from
...
- [x] 4.4 Update `docs/architecture/typoscript-and-site-sets.md` if it
