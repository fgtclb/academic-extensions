...
- [x] 1.1 Confirm that `ace-716-item-and-list-partials` and the change for
...
- [x] 2.1 Declare `plugin.tx_academicpersons.viewMode.allowed` and
  ...
  `SiteSetDeliveryTest`.
- [x] 2.2 Add the mode resolution to `ProfileController` and assign
...
- [x] 2.3 Relabel the item of the value `list` to "Tiles" and "Kacheln" for
...
- [x] 3.1 Add list, selected-profiles and selected-contracts tests with the
  default mode `table`, asserting `<table` and the five header labels. Record
  that they fail today (the grid renders).
- [x] 3.2 Add tests for the switch: links for both modes and `aria-current` on
...
- [x] 3.3 Add a test that the page 2 link of a table list carries
  `viewMode`, and record its failure.
- [x] 3.4 Add a fixture extension with a `contact` mode (TSconfig item,
  `Profile/ViewMode/Contact.html` and the allow-list). Test that it renders,
  and record its failure.
- [x] 3.5 Move the grid into `Profile/ViewMode/List.html`, add `Table.html`,
  ...
  Done: `AcademicPersonsListViewModesTest` (22 tests, fixture extension
  `test_profile_view_modes`). Against `main` 15 of them fail - every table,
  switch, navigation and project mode test; the 7 that pass assert what holds
  today as well (tiles, ignored requests, the card).
- [x] 3.6 Add a card test that the output is unchanged and that no view mode
  field is offered.
  Done in two halves: the rendering in `AcademicPersonsListViewModesTest`, the
  form in `Backend/FormEngine/ViewModeFieldsTest`, which compiles the backend
  form with the card's page TSconfig. The same test covers 2.3 and the
  project's TSconfig item of 3.4.
- [x] 3.7 Hard-code the grid again on purpose and watch 3.1 go red; restore.
  Red for the list; also red: the pattern check removed (the `../Table` case
  errors on the missing partial) and the switch check removed.

## 4. Routes

- [x] 4.1 Add the `/view-mode/{viewMode}` routes (alone, with
  ...
  Done in a class of its own, `Routing/ProfileViewModeRouteTest`: it renders
  the shipped templates and imports the shipped files through `imports:`,
  where `ProfileRouteEnhancerTest` renders `EXT:test_plugin_templates` and
  inlines them. Without the routes 6 of its 11 tests fail.
- [x] 4.2 Verify that `/{profile_name}` of ListAndDetail and the existing page
  and letter routes still resolve next to the new routes.
- [x] 4.3 Verify with a fixture site configuration that a project mode can be
...
- [x] 5.1 Add `Documentation/Changelog/3.0/Feature-ListViewModes.rst` from
...
- [x] 5.2 Document both settings, the switch, the view mode routes and the
...
- [x] 5.3 Update `docs/architecture/typoscript-and-site-sets.md` if it
...
- [x] 6.1 After implementation, file the ACE issue in YouTrack, verify its key
  ...
  Filed as ACE-735 (Story, 3.0.0), linked to the five project issues.
- [x] 6.2 Commit as `[FEATURE] ACE-<NNN>: Render the list view modes` in TYPO3
  Core format.
...
- [x] 7.1 `composerUpdate`, then `lintPhp`, `cgl -n`, `phpstan`, `unit` and
  ...
  routing tests.
  Done: 2779 functional tests; PostgreSQL for the routing, view mode,
  navigation state and backend form classes (74 tests), on v14 as well.
- [x] 7.2 `composerUpdate`, then the same suites green with `-t 14`.
  Done: 2795 functional tests.
...
- [x] 7.5 No backport: a new feature. State it in the pull request, together
