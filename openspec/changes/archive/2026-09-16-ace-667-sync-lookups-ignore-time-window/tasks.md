...
- [x] 1.1 Add a functional test for `academic:updateprofiles`: a profile with
...
- [x] 1.2 Add the same for a start time in the future and for a profile
  restricted to a frontend user group. Show both fail before the change.
- [x] 1.3 Add a functional test for `academic:createprofiles`: a frontend
  ...
  unselected.
- [x] 1.4 Add a test for the enable field list: with a TCA override that
...
- [x] 1.5 Add or extend functional tests proving that the "show hidden
...
- [x] 2.1 Add the sync-only helper and use it in `findByFrontendUser()` for
...
- [x] 2.2 Remove the start and end time restrictions in both frontend-user
  queries. Verify 1.3.
...
- [x] 3.2 Extend `docs/architecture/frontend-user-contact-import.md` with the
  lookup rule (which enable fields the sync ignores, and that it never writes
  them). Verify `lintMarkdown -n`.

## 4. File the issue

- [x] 4.1 After implementation, file the ACE issue in YouTrack as a
...
- [x] 5.2 `functional` also on PostgreSQL for the new sync tests, because
  they write.
