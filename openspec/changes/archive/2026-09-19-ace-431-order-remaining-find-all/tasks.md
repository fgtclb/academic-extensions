...
- [x] 1.3 Mutations: drop the ordering — red on PostgreSQL for all four,
      green on SQLite; order by `sorting` instead — red on SQLite for the three
      contact repositories, green for profile information, whose fixture does
      not run `sorting` against uid order. Proven on v13 and v14.

## 2. Definition of done

- [x] 2.1 `lintPhp`, `cgl -n`, `phpstan`, `unit` and `functional` green on
      TYPO3 v13 and v14, each after its own `composerUpdate`; the persons
      repository tests on PostgreSQL for both.
- [x] 2.2 `Documentation/Changelog/3.0/` entry in `academic_persons`;
