...
- **BREAKING** (markup): images change from `<img>` to `<picture>`, and the
  fallback image loads lazily and states the dimensions of its preset. This is
  announced as a Breaking changelog entry per extension.
...
this today. Filed as ACE-710 after the implementation was green. Depends on the
change `ace-646-responsive-image-partial`.
