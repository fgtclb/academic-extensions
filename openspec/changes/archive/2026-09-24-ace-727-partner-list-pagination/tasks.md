...
- [x] 1.1 Confirm `ace-723-list-filter-get-urls` is merged; the pagination
  links build on its GET shape.

## 2. Configuration

- [x] 2.1 Split the FlexForm: `MapSettings.xml` with today's fields for the
...
- [x] 3.2 Paginator and pagination in `listAction()`, `Partner/Pagination.html`
  ...
  render).
- [x] 3.3 Regression: pagination off renders all five partners and no
...
- [x] 3.4 Both pagination classes: one test with `numbered_pagination`
  loaded, one without; show the second fails when `NumberedPagination` is
  used unconditionally.

## 4. Documentation

- [x] 4.1 `docs/architecture/`: the list demand URL section names
  `currentPage`; linked from `docs/architecture/Index.md`.
- [x] 4.2 `academic-partners` `Documentation/Configuration/` for the three
  settings and
...
- [x] 5.1 After implementation, file the ACE issue in YouTrack, rename the
  change to `ace-727-partner-list-pagination`, and commit as
  `[FEATURE] ACE-727: Paginate the partner list` in TYPO3 Core format.
...
- [x] 6.4 `docs/` and the `academic-partners` `Documentation/` changelog
