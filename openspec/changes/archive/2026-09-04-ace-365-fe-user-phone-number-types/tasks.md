...
- [x] 4.2 User runs `composerUpdate`, `phpstan`, `unit`, and `functional` separately for TYPO3 v13 and v14, plus `lintPhp`, `cgl -n`, `lintMarkdown -n`, RST rendering, and the synchronisation tests on SQLite and PostgreSQL; record every result before declaring the change complete.
- [x] 4.3 After the user reports successful final tests and explicitly approves commits, create the TYPO3 Core-style commit `[FEATURE] ACE-365: Configure imported phone number types`, carrying the telephone-only contract fix with it, and with no unrelated files.
