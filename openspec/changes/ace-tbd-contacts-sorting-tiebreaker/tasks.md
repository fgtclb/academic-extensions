...
- [ ] 1.1 Re-check the premises of `design.md`, *Context*, on `main`.
- [ ] 1.2 Functional test of `ContactRepository::findByPid()` with a fixture
...
- [ ] 1.3 Run it without the tiebreaker on SQLite and on PostgreSQL, for v13
      and v14, and record the result per database in the test docblock —
      SQLite cannot fail it.

## 2. Implementation

- [ ] 2.1 Append `'uid' => QueryInterface::ORDER_ASCENDING` to the orderings
      of `findByPid()`, with a comment naming rule 3. Verify 1.2 passes.
- [ ] 2.2 Mutation: drop it, watch 1.2 go red where 1.3 said it would; restore.
...
- [ ] 3.1 `lintPhp`, `cgl -n`, `phpstan`, `unit` and `functional` green on
      ...
      PostgreSQL as well.
- [ ] 3.2 `Documentation/Changelog/3.0/Important-*.rst` in
      `academic_contacts4pages`; `docs/` needs nothing, rule 3 already covers
      the case.
