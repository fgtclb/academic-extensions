...
- [ ] 1.1 Add a functional test for `academic_partners` that compiles the
  ...
  red proof from `main`.
- [ ] 1.2 Add a scenario asserting the empty placeholder entry is first, and
  verify it passes before and after the change.
- [ ] 1.3 Add a scenario with a title starting with a diacritic (`Ö` between `O`
  and `P`) and verify it fails on the unchanged configuration for the ordering
  reason, not for a missing record.
- [ ] 1.4 Add a scenario asserting hidden and deleted partner pages stay absent
  from the entries, and verify it passes before and after.
- [ ] 1.5 Verify the new test runs on TYPO3 v12 **and** v13, each after its own
...
- [ ] 2.2 Take the red proof by mutation once more if the line arrived by
  ...
  uncommitted.
- [ ] 2.3 Verify the diff contains nothing but that declaration, the test and
...
- [ ] 3.2 Check `docs/` on this branch for a statement about backend select
  ordering and update it if one exists; state in the pull request when nothing
  needed a change.

## 4. Pull request

- [ ] 4.1 Rebase #642 onto the merged change, amend it with the test and the
...
- [ ] 4.2 Add the `Releases: 2` footer, and link the pull request on ACE-669
  next to the `main` one.
- [ ] 4.3 Archive the change as the last commit of that same pull request.
...
- [ ] 5.3 `functional -d postgres` for `academic_partners` on both core
...
- [ ] 5.5 The extension's `Documentation/Changelog/2.4/` entry written, and
  `docs/` checked.
- [ ] 5.6 Commit message in TYPO3 Core format with the verified `ACE-669`
