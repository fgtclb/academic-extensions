...
- [ ] 1.1 Add a functional test for `academic_partners` that compiles the
...
- [ ] 1.2 Add a scenario asserting the empty placeholder entry is the first
  entry, and verify it passes before and after the change — it pins behaviour
  the change must not break.
- [ ] 1.3 Add a scenario with a title starting with a diacritic (`Ö` between
  `O` and `P`) and verify it fails on the unchanged configuration for the
  ordering reason, not for a missing record.
- [ ] 1.4 Add a scenario asserting hidden and deleted partner pages are still
  absent from the entries, and verify it passes before and after.

## 2. Implementation

- [ ] 2.1 Declare the ascending label order on the partner field of
...
- [ ] 2.2 Verify the diff of the extension contains nothing but that
...
- [ ] 3.1 Add `packages/fgtclb/academic-partners/Documentation/Changelog/2.4/Feature-PartnerSelectIsSortedByLabel.rst`
...
- [ ] 3.2 Check `docs/` for a statement about backend select ordering and
  update it if one exists; state in the pull request when nothing needed a
  change.

## 4. Pull request

- [ ] 4.1 Rebase #641 onto the merged change, amend it with the test and the
...
- [ ] 4.2 Archive the change as the last commit of that same pull request.

## 5. Backport to branch `2`

- [ ] 5.1 Write the file level backport analysis
  (`docs/workflow/backporting.md`, the three measurement commands) into
  `.agent/reports/`, and state which files are byte-identical.
- [ ] 5.2 Create the change on branch `2` with `openspec new change` — never a
...
- [ ] 6.3 `functional -d postgres` for `academic_partners` on both core
...
- [ ] 6.5 The extension's `Documentation/` changelog updated in the same
  change, and `docs/` checked.
- [ ] 6.6 Commit message in TYPO3 Core format with the verified `ACE-669`
