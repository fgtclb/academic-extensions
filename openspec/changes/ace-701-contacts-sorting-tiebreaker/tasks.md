...
- [x] 1.1 Re-check the premises of `design.md`, *Context*, on `2`.
...
- [x] 1.3 Run it without the tiebreaker on SQLite and on PostgreSQL, for v12
      and v13, and record the result per database in the test docblock —
...
- [x] 3.1 `lintPhp`, `cgl -n`, `phpstan`, `unit` and `functional` green on
      TYPO3 v12 and v13, each after its own `composerUpdate`; the new test on
      PostgreSQL as well.
