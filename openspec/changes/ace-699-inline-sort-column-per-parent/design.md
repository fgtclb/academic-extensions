...
well; what follows is the measurement that says so, and the five places the
patch had to be adapted.

Measured on this branch at `98de3bbd3` against `main` at `f04f74dd8`:

| Files                                                    | State                                                      |
|----------------------------------------------------------|------------------------------------------------------------|
| the three `ext_tables.sql`, the four TCA relation blocks | the touched blocks are **byte-identical**                  |
| `docs/architecture/database-queries.md`, `AGENTS.md`     | differ throughout - v12 notes, a different rule 1          |
| `Documentation/Changelog/2.4/`                           | exists in all three extensions and is globbed              |
| `academic-persons/Documentation/Upgrade/Index.rst`       | does not exist here, so the wizard is not added to a guide |

The APIs the change uses were checked against the installed v12.4.45 tree
rather than recalled: `RelationHandler::writeForeignField()` writing
`foreign_sortby` (`RelationHandler.php:1036-1059`), `DefaultTcaSchema` deriving
only `ctrl.sortby` (`:206`), `AbstractSchemaManager::listTableColumns()` on
DBAL 3.10.6 (`:213`), `QueryBuilder::set()` with a fourth `$type` argument
(`:751`), `Install\Attribute\UpgradeWizard`, both DataHandler hook entry
points, and the two Extbase persistence events. All present.

What had to be adapted:

1. **PHP 8.1.** The three upgrade wizards are `final class` with a promoted
   `private readonly` property rather than `final readonly class`.
2. **`$GLOBALS['LANG']` in the DataHandler tests.** On TYPO3 v12 the
   DataHandler writes its log through `BackendUtility::getLanguageService()`,
   which returns `$GLOBALS['LANG']` and is typed against it, so a data map run
   without one dies with a `TypeError`. The three test classes set it, the way
   `AddressRecordSelectionTest` already does here.
3. **`academic_partners` gains `typo3/cms-install`.** It ships its first
   upgrade wizard here, and its base functional test case loads the extension,
   exactly as `academic_projects` does.
4. **`academic_contacts4pages` has no `Classes/Hook/` here** - the localization
   policy hook is `main`-only - so the directory and the `ext_localconf.php`
   registration are new rather than an addition to an existing block.
5. **`tx_academicpersons_domain_model_contract` is not workspace aware here**
   (that arrived with 3.0), so the wizard and hook docblocks of
   `academic_persons` do not mention workspace rows. The partnership and
   contact tables are workspace aware on both branches.
...
owning parent. Measured on `main` and reproduced here: a partnership created on a partner page
with a role selected is stored with `sorting = 3` and `role_sorting = 0`. Without a
...
derives the `ctrl.sortby` column and nothing else, on v12.4.45
(`DefaultTcaSchema.php:206`) as on v13.4.35 (`:218`).
...
- [Upgrade wizards are among the v15 blockers] → A wizard is still the
  documented way on v12 and v13; it uses the same API as the existing
  ones.

## Backport

None. This branch is the backport target; `main` carries the change already.
