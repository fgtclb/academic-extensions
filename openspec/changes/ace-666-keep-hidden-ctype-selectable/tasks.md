...
- [x] 1.1 Add a fixture extension for the academic_base functional tests that
...
- [x] 1.3 Add the negative cases to the same test: a new record, an existing
...
- [x] 1.4 Add a DataHandler test that saves the unchanged hidden academic type
...
- [x] 1.5 Implement the provider, its registration in a new
...
- [x] 2.1 Add the behaviour to the hiding section of
      `docs/architecture/typoscript-and-site-sets.md`, and verify the page
      keeps its `## See also`.
- [x] 2.2 Document it in the academic_base `Documentation/` next to the
...
- [ ] 2.3 Prepare a forge report for the general core behaviour (a stored
      ...
      exists. Open: the report is drafted for the maintainer and not filed, so
      the Important entry carries no forge reference yet.
...
- [ ] 3.1 Backport: separate change on branch `2` after a backport analysis
      ...
      here. Open by design: it is not part of this pull request.

## 4. File the issue

- [x] 4.1 File the ACE issue in YouTrack, rename the change to
      `ace-666-keep-hidden-ctype-selectable`, and commit as
      `[BUGFIX] ACE-666: Keep hidden academic content types` in TYPO3 Core
      format.
