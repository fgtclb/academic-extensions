...
- [ ] 1.1 Rewrite the four header tests (`listPluginRendersContentElementHeader`,
...
- [ ] 1.2 Add a case per plugin with the header layout "Default": the header
  once, and no `header` element inside the plugin's wrapper. Record that it
  fails today on the empty element.
- [ ] 1.3 Add a case with the header layout "Hidden": no header. It passes
  today; say so, it pins the layout's behaviour.
- [ ] 1.4 Add a case with the switch on and a fixture content element layout
...
- [ ] 2.1 The constant and setting `renderContentElementHeader` (default `0`)
...
- [ ] 2.2 Wrap the `Header/All` line of `Job/List.html`, `Job/Show.html`,
...
- [ ] 3.1 `Documentation/Changelog/2.4/Important-*.rst` in both extensions:
...
- [ ] 3.2 Document the setting in both extensions' `Documentation/`
  configuration chapter: what it does and when a site needs it.
- [ ] 3.3 `docs/architecture/content-element-rendering.md`: who renders a
...
- [ ] 4.1 After implementation, file the ACE issue (`[3.x][2.x]`, Bug,
