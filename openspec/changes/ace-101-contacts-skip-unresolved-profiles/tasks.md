...
- [x] 1.1 Add a plugin fixture to `AcademicContacts4PagesListPluginTest` with
...
- [x] 1.2 Add the same fixture with `settings.showHiddenRecords` enabled plus a
  hidden contact row pointing at a visible profile; assert that contact is
  rendered and the hidden-profile contact is not.
- [x] 1.3 Add a functional test for the page contacts data processor that
...
- [x] 2.1 Add `PageContactsProvider` and the `PageContacts` value object as
  `final readonly` classes; verify with a functional test of the provider
  that the role split matches the shown contacts.
- [x] 2.2 Use the provider in `ContactsController::listAction()`, keep the
  address record provider handling, and verify tests 1.1 and 1.2 pass on v13
  and v14.
- [x] 2.3 Make the processor a public service with the provider injected and
  verify test 1.3 passes, and that the existing repository and site set tests
  stay green.

## 3. Documentation

- [x] 3.1 Add `Documentation/Changelog/3.0/Important-ContactsWithoutVisibleProfileAreSkipped.rst`
...
- [x] 3.2 Update the plugin and processor chapter of the extension's
...
- [x] 4.1 After implementation, confirm with the maintainer whether the change
