...
- [x] 1.1 Add `Configuration/Sets/Full/settings.definitions.yaml` with
...
- [x] 2.1 Add the stateless `ProgramFactsBuilder` and the `ProgramFact`
...
- [x] 2.2 Ahead of pull request #617 (ACE-591), which is still open (the
      maintainer's decision at apply, see design.md), add the Font Awesome
...
- [x] 2.3 Add `factsFields` to the `program-data` processor and render
...
- [x] 2.4 Render `Program/Facts` in `Templates/Details/Show.html`; a
      functional plugin test with the same setting asserts the order (today
      no credit points).
- [x] 2.5 Render the card facts in `Partials/Program/Item.html`; a
      ...
      only.
- [x] 2.6 Functional tests for the empty setting assert the output of page
      and details element is unchanged against today's fixtures.
- [x] 2.7 Functional test that the category type facts follow the category
...
- [x] 3.1 Add a functional test with a fixture partial root that overrides
      ...
      before 2.3 and 2.4.
- [x] 3.2 Delete `Resources/Private/Partials/Program/Categories.html`; grep
...
- [x] 4.1 Document both settings, the vocabulary and the per-place defaults
      in `academic_programs` `Documentation/` and in `docs/`, linked from the
      section `Index.md`.
- [x] 4.2 Add `Documentation/Changelog/3.0/Feature-ProgramFactsFieldList.rst`
...
- [x] 6.3 `docs/` and the `Documentation/` changelog updated; `README.md`
