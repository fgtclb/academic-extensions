# academic-jobs/label-overrides Specification
...
- **THEN** the job form shows that label

#### Scenario: Label set for one plugin

- **WHEN** a site sets the label of a job type option under
  `plugin.tx_academicjobs_newjobform._LOCAL_LANG`
- **THEN** the job form shows that label on TYPO3 v12 and v13
...
- **WHEN** a site sets the label of a job type option only under
