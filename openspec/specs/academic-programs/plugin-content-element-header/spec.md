# academic-programs/plugin-content-element-header Specification

## Purpose
Defines who renders the header of the plugins of `academic_programs`, and how
often it appears.

## Requirements

### Requirement: The plugin header renders once by default
Without configuration, the program list, program details and program finder
...
element layout, for every header layout except "Hidden", which shows none. This
...
An integrator SHALL be able to switch on, per site, that the program list,
program details and program finder plugins render the header and the subheader
of their content element themselves, above their output, for sites whose
content element layout does not render it. Switched on, the header SHALL render
for every header layout except "Hidden", including "Default". This applies to
