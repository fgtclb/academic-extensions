# academic-programs/image-rendering Specification
...
#### Scenario: Program page of a site whose page object is a PAGEVIEW
- **WHEN** a visitor opens a program page with an image on a site whose page
  object resolves its templates from `paths` rather than from
  `templateRootPaths`
- **THEN** the page shows the same `<picture>` as on any other site
