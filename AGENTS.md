...
- **`tasks.md` ends with this repository's definition of done**, not with the
  last code change: the gates for both TYPO3 v12 and v13, `docs/`, the
  extension's `Documentation/Changelog/2.4/` entry, and the commit message. An apply
...
- **Specs are branch-scoped.** Never sync `openspec/specs/` or move an
  archived change between `main` and `2`; a backport arriving from `main` is a
  change of its own here, re-derived from the backport analysis — see
...
- `packages-dev/testing-helper/` — `fgtclb/academics-monorepo-testing-helper`: shared functional-test traits. Five of them; see [Testing helper](docs/testing/testing-helper.md) rather than a list here that goes stale.
