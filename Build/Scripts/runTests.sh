...
# Composer authentication, forwarded into the container it runs in.
#
...
# environment, so the token never lands on a command line and cannot show up in
# the process list of the host.
COMPOSER_AUTH_ENV=""
if [ -n "${COMPOSER_AUTH:-}" ]; then
    COMPOSER_AUTH_ENV="-e COMPOSER_AUTH"
fi
USERSET=""
...
# 530 MB, PHP about 220 MB). No chunk can be faster than the heaviest test class, which is
# never split; beyond that point more chunks cost nothing but containers.
...
case ${TEST_SUITE} in
    ...
    composer)
        COMMAND=(composer "$@")
        ${CONTAINER_BIN} run ${CONTAINER_SIMPLE_PARAMS} --name composer-command-${SUFFIX} -e COMPOSER_CACHE_DIR=.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${COMPOSER_AUTH_ENV} ${IMAGE_PHP} "${COMMAND[@]}"
        SUITE_EXIT_CODE=$?
        ;;
    composerUpdate)
        rm -rf .Build composer.lock composer.json.orig
        \cp -f composer.json composer.json.orig
        ${CONTAINER_BIN} run ${CONTAINER_SIMPLE_PARAMS} --name composer-command-${SUFFIX} -e COMPOSER_CACHE_DIR=.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${COMPOSER_AUTH_ENV} ${IMAGE_PHP} composer require --dev --no-update "typo3/minimal":"^${CORE_VERSION}"
        SUITE_EXIT_CODE=$?
        if [[ "${SUITE_EXIT_CODE}" -eq 0 ]]; then
          ${CONTAINER_BIN} run ${CONTAINER_SIMPLE_PARAMS} --name composer-command-${SUFFIX} -e COMPOSER_CACHE_DIR=.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${COMPOSER_AUTH_ENV} ${IMAGE_PHP} composer install
          SUITE_EXIT_CODE=$?
        fi
    ...
    functional|seedManifest)
        ...
        if [[ "${TEST_SUITE}" == "functional" && -z "${FUNCTIONAL_CHUNK}" && ${FUNCTIONAL_PARALLEL} -gt 1 ]]; then
            ...
            # chunk. Every chunk gets its own suffix - ending in its chunk number - and with
            # it its own container network, database container and instance directory, so
            # the chunks cannot see each other. The files of the run live in a directory of
            # its own, because two runs in one checkout must not overwrite each other's
            # chunks.
            #
        ...
        case ${DBMS} in
            mariadb)
                ...
                SUITE_EXIT_CODE=$? && [[ "${SUITE_EXIT_CODE}" -ne 0 ]] && printSummary
                ${CONTAINER_BIN} run --name mariadb-func-${SUFFIX} --network ${NETWORK} -d -e MYSQL_ROOT_PASSWORD=funcp --tmpfs /var/lib/mysql/:rw,noexec,nosuid ${IMAGE_MARIADB} --skip-log-bin --innodb-flush-log-at-trx-commit=2 --innodb-doublewrite=0 >/dev/null
                SUITE_EXIT_CODE=$? && [[ "${SUITE_EXIT_CODE}" -ne 0 ]] && printSummary
            ...
            mysql)
                ...
                SUITE_EXIT_CODE=$? && [[ "${SUITE_EXIT_CODE}" -ne 0 ]] && printSummary
                ${CONTAINER_BIN} run --name mysql-func-${SUFFIX} --network ${NETWORK} -d -e MYSQL_ROOT_PASSWORD=funcp --tmpfs /var/lib/mysql/:rw,noexec,nosuid ${IMAGE_MYSQL} --skip-log-bin --innodb-flush-log-at-trx-commit=2 --innodb-doublewrite=0 >/dev/null
                SUITE_EXIT_CODE=$? && [[ "${SUITE_EXIT_CODE}" -ne 0 ]] && printSummary
            ...
            postgres)
                ...
                SUITE_EXIT_CODE=$? && [[ "${SUITE_EXIT_CODE}" -ne 0 ]] && printSummary
                ${CONTAINER_BIN} run --name postgres-func-${SUFFIX} --network ${NETWORK} -d -e POSTGRES_PASSWORD=funcp -e POSTGRES_USER=funcu --tmpfs /var/lib/postgresql/data:rw,noexec,nosuid ${IMAGE_POSTGRES} -c fsync=off -c synchronous_commit=off -c full_page_writes=off >/dev/null
                SUITE_EXIT_CODE=$? && [[ "${SUITE_EXIT_CODE}" -ne 0 ]] && printSummary
    ...
    lintPhp)
        ...
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name lint-php-${SUFFIX} -e COMPOSER_CACHE_DIR=.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${COMPOSER_AUTH_ENV} ${IMAGE_PHP} /bin/sh -c "${COMMAND}"
        SUITE_EXIT_CODE=$?
        ;;
    lintMarkdown)
    ...
    phpstan)
        ...
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name phpstan-${SUFFIX} -e COMPOSER_CACHE_DIR=.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${COMPOSER_AUTH_ENV} ${IMAGE_PHP} "${COMMAND[@]}"
        SUITE_EXIT_CODE=$?
        ;;
    phpstanGenerateBaseline)
        ...
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name phpstan-baseline-${SUFFIX} -e COMPOSER_CACHE_DIR=.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${COMPOSER_AUTH_ENV} ${IMAGE_PHP} "${COMMAND[@]}"
        SUITE_EXIT_CODE=$?
        ;;
    unit)
