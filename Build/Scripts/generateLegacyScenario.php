...
function main(array $argv): int
{
    ...
    $target = SET_PATH . '/ScenarioLegacy.yaml';
    ...
    $rendered = render(buildRoot($scenario['entities']['page'][0]));
    ...
    if ($check) {
        ...
        fwrite(STDERR, sprintf("%s differs from what \"Scenario.yaml\" produces.\n", basename($target)));
        return 1;
    }
...
function buildRoot(array $sourceRoot): array
{
    ...
    $root['entities'] = ['template' => [['self' => templateRecord()]]] + ($root['entities'] ?? []);
 ...
 */
function render(array $root): string
{
    $lines = explode("\n", rtrim(fileHeader(), "\n"));
    $lines[] = '';
 ...
 */
function templateRecord(): array
{
    $staticFiles = implode(',', [
        // NOT the theme. EXT:bootstrap_package delivers everything it has
        // through site sets and nothing through a static template since version
        // 16 removed "Configuration/TypoScript/" and the addStaticFile() call
        // that offered it - and the "core-14" instance runs 16 while "core-13"
        // still runs 15. A "sys_template" naming the theme would therefore
        // render one instance and nothing at all in the other.
        //
        // So the "/legacy/" tree is not themed. It gets the smallest page object
        // that puts the content of a page on the page, shipped by the seed
        // package itself in both delivery forms - see
        // "packages-dev/dev-site/Configuration/TypoScript/setup.typoscript".
    ...
    // EXT:felogin is not in that list and does not need to be: it registers its
    // TypoScript through ExtensionManagementUtility::addTypoScriptSetup() with
    // "includeInSiteSets = false", which reaches every "sys_template" driven
    // site and no site set at all. The "/" site has to name "typo3/felogin" in
    // its "dependencies" for the same reason, mirrored.

    $constants = <<<'TYPOSCRIPT'
        # What "config/sites/academics/settings.yaml" says through site settings, said
        # here the way a site without site sets says it - with the page uids of this
        # tree, so a list of this tree links to the detail page of this tree.
        #
        # The two "page.theme.*" settings of that file have no counterpart here: they
        # belong to EXT:bootstrap_package, and this tree is not themed by it - see the
        # "include_static_file" above.
