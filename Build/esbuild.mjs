...
for (const extension of found) {
    for (const pass of passes) {
        ...
        await build({
            ...
            // Everything is bundled: stylesheets because "@use" and url() have
            // to be resolved at build time, scripts because the result has to
            // be a classic script.
            //
            // This is where this branch differs from "main" deliberately, and
            // it is the one difference worth knowing about. There, scripts are
            // emitted unbundled as ES modules and resolved in the browser
            // through the TYPO3 import map, which gives each one a "?bust="
            // cache key. That cannot work here: the Fluid view helper for a
            // module, "f:asset.module", does not exist on TYPO3 v12 — v12.4
            // ships only the css and script view helpers — and no TypoScript
            // key can load a module either. An ES module reached through a
            // classic script tag simply does not execute.
            //
            // So this branch emits one self-contained IIFE per entry point,
            // which "f:asset.script" and "page.includeJSFooter" load exactly as
            // they loaded the hand written files before.
            bundle: true,
            ...(isScript ? { format: 'iife', platform: 'browser', tsconfig: resolve(buildRoot, 'tsconfig.json') } : {}),
