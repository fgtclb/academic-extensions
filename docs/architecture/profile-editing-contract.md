...
- **A component can be handed the contract.** An element below the root neither
  knows nor asks which element the attributes came from — which is what the
  custom elements need, and why the reader exists before the elements do.
...
**CropperJS 1.6.1** comes from `EXT:core`, which maps it as `cropperjs`, and is
...
markup, so a rendering base class would have nothing to render and would have to
be kept away from the children Fluid put below the element; without one there is
nothing to keep away, and the only code that ever writes below an element is
that element.

What the base does, in full:

- An assignment to a property schedules **one** update, in a microtask.
  Several assignments in the same turn become one update.
- The update reports **which properties changed**, each with the value it had
  before, to `updated()`. `firstUpdated()` runs once, before the first of them.
- `updateComplete` settles after `updated()` ran, and immediately when nothing
  is scheduled. It **rejects** when a callback threw — an element whose update
  failed has no markup for the caller to work with, and `documents.ts` awaits
  it inside the `try` of `openDocument()`, so a promise that never settled
  would take the error path and the teardown with it. It resolves `false` when
  a callback assigned a property and queued the next update, and `true`
  otherwise. The document editor overrides `getUpdateComplete()` to await its
...
- A property an owner assigned **before the element was upgraded** is an own
  ...
  constructor takes those off the instance and the first update hands them back.

`Tests/JavaScript/element-lifecycle.test.ts` pins all of it but the last
bullet, which the four `*-element-upgrade.test.ts` files pin instead.

`hasUpdated` and `firstUpdated()` are the two parts of it that no element uses
yet: none of the five implements the callback and nothing reads the flag. They
are kept deliberately, as the seam an element that has to set something up once
will use, and they are pinned so that an unused seam cannot rot.

**Why a base class at all**: the document editor is the element that needs every
...
the two classes that declare properties extend `ProfileEditingElement<Self>`, so
a mistyped name is a `typecheckJs` error rather than a property that silently
drops out of the changed map. There are no decorators anywhere: the behavioural
suite runs these sources under node's type stripping, which erases annotations
...
The editor imports no user interface library at all. The only bare specifiers
its modules use are the ones of this repository, which the TYPO3 import map
answers, plus CKEditor 5 from `EXT:rte_ckeditor`. There is no version to pin
against the core the site runs, nothing to keep in step between the browser and
the behavioural suite, and no rendering engine that could reach the markup
Fluid produced.
...
decorator, and the two elements below it that have properties declare them as
accessor pairs. The behavioural suite runs the TypeScript sources under node's type
...
- **It is called `applyState()` and not `render()`.** This element renders
  nothing; it writes attributes, classes and `hidden` onto markup Fluid
  produced.
...
An open that fails removes it straight away instead, through the same teardown,
and reports the failure like a refused request — an override of
`Prototypes.html` that drops a prototype throws in the element's update, and an
editor that cannot build its panel would fail the same way on every render that
followed.

**A rebuild is a decision, not a default.** The `ChangedProperties` map handed
to `updated()` separates the structural change — `fields`, `mode`, `kind`, `record`, `heading`,
...
"Show in frontend"), and a pressed "Show" would announce the opposite of what
it does — the state is said in words by the `Hidden` tag that precedes the
action group instead. The `hidden` key of the `contact-row` prototype does two
things at once — it marks the row for the stylesheet through `data-pe-attr`,
which dims it through the secondary text colour rather than `opacity` so that
...
TYPO3: CKEditor 5 by `EXT:rte_ckeditor`, and CropperJS by `EXT:core`, which maps
