...
v12 and v13, and so does that template variable.
...
Each of the three extensions registers its own listener with the
`event.listener` tag in its `Configuration/Services.yaml`, and passes two
constants: the page type it owns and its category group.

The tag and not an attribute: TYPO3's `Core\Attribute\AsEventListener` **does
not exist on TYPO3 v12**, which this branch supports — `Core\Attribute\` holds
only `AsAllowedCallable` and `WebhookMessage` there. Symfony's attribute of the
same name is not a substitute, because it registers nothing in TYPO3: the
listener would silently never fire. On `main`, which starts at v13, the same
three listeners use the attribute.
...
- [Dependency injection](dependency-injection.md) — why the listeners are
  registered with the `event.listener` tag on this branch.
