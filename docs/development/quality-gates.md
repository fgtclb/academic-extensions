...
versions — the `lint` job. It is also the cheapest way to
...
in the `phpstan` arm. PHPStan analyses the sources **against
...
, so a test that cannot work on one DBMS can
...
; the rendering itself is in `executeRstRendering()`.
...
| `functional-dbms`   | unit               | the same four pairs × 4 DBMS, 16 jobs |
...
staging stopped a run 8 times, while it added the whole SQLite stage — some 5
minutes on this branch before ACE-692, some 2.5 after it — to the critical path
of every run that went on to pass. A defect found on SQLite now costs the DBMS
jobs of that run as well.
...
out in the workflow header of `ci.yml`.
...
this branch — `ci.yml`, `pr-comment.yml`, `publish.yml`, the twelve package
publish workflows and the extension template — and is changed in all of them
together. `nightly.yml`, which starts the nightly run of this branch, lives on
`main` and carries the same label there.
...
* `functional` green for the same versions whenever the change can affect
  ...
  other, and a serial v13 run takes some 7 minutes on SQLite and some 27 on
  MySQL where `-j 16` takes under 1 and 11 — see
