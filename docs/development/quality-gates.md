...
The PHP version is not free to choose per core version: TYPO3 v13 needs PHP 8.2
or newer, so `-t 12` is run with 8.1 above and `-t 13` with 8.2 — the lowest
supported one in each case, which is also what continuous integration uses.
...
| `phpstan`                                         | PHPStan, level 8 | `Build/phpstan/Core12/`, `Build/phpstan/Core13/` | **yes**                     |
...
`Build/Scripts/runTests.sh:596-605` runs php-cs-fixer with
...
only reports, which is the form CI uses (`.github/workflows/ci.yml:111-112`).
...
(`Build/php-cs-fixer/header-comment.php:26-44`).
...
> extensions" (`.github/workflows/ci.yml:114-116`). Running it locally without
...
Xdebug off (`Build/Scripts/runTests.sh:715-723`). It has no configuration file;
...
which is why CI runs it without `composerUpdate` and across all five PHP
versions (`.github/workflows/ci.yml:158-175`). It is also the cheapest way to
...
`Migrations/*` and the source and test folders of the *other* core version
(`Build/phpstan/Core12/phpstan.neon:11-20`). Three extension packages are
...
(`Build/Scripts/runTests.sh:739-744`). PHPStan analyses the sources **against
the core that is installed in `.Build/`**, so the same code produces different
findings on v12 and on v13: a method that exists only in one of them, a
...
core version (`.github/workflows/ci.yml:118-156`).

The two configurations differ in exactly one place, and it is the reason they
have to be a pair:

| File                    | Core12                                          | Core13                                          |
|-------------------------|-------------------------------------------------|-------------------------------------------------|
| `phpstan.neon`          | excludes `Classes/Core13/*`, `Tests/*/Core13/*` | excludes `Classes/Core12/*`, `Tests/*/Core12/*` |
| `phpstan-baseline.neon` | its own findings, 226 lines                     | its own findings, 236 lines                     |

A class written for one core version cannot be analysed against the other one —
it references API that is not installed — so each configuration excludes the
folders belonging to the version it is not running for. `academic-base` is the
only extension with such folders today: `Classes/Core12/`, `Classes/Core13/`
and the matching `Tests/Functional/Core12/` and `Tests/Functional/Core13/`.

The `paths` entry is `packages/` as a whole, so a new source folder is picked
up without a change. A new *core-version-specific* one is not: it has to be
added to the `excludePaths` of the other version's configuration, or that
configuration will analyse it against the wrong core.

> [!NOTE]
> `Build/phpstan/Core12/phpstan-constants.php` and its `Core13` counterpart
> are identical leftovers from TYPO3 Core's configuration. Neither
> `phpstan.neon` references them, so nothing loads them.

### The baseline

Each core version has its own `phpstan-baseline.neon`, included from its
`phpstan.neon:2`. It is regenerated per version:
...
`--allow-empty-baseline` (`Build/Scripts/runTests.sh:745-750`).
...
(`Build/phpstan/Core12/phpstan.neon:9`) and is **shared by both core
...
(`Build/Scripts/runTests.sh:751-762`, `663-714`). `unitRandom` adds
...
TYPO3 deprecations. Code that calls a v13-deprecated API turns the suite red on
v13 even when it behaves correctly — which is intended. A deprecation therefore
has to be resolved in a way that still works on v12, or it has to wait until
v12 support is dropped; working around the flag is not an option.
...
```bash
Build/Scripts/runTests.sh -t 12 -s functional \
...
the container afterwards (`Build/Scripts/runTests.sh:663-714`). With the
...
```bash
Build/Scripts/runTests.sh -t 12 -s functional -d postgres -i 16
```

`functional` also excludes the group `not-${DBMS}`
(`Build/Scripts/runTests.sh:665`), so a test that cannot work on one DBMS can
...
## Core version aware tests: `not-core-12` and `not-core-13`

Every PHPUnit suite is started with `--exclude-group not-core-${CORE_VERSION}`
(`Build/Scripts/runTests.sh:665`, `753`, `759`). The name reads as an
exclusion, so the effect is inverted from what it looks like:

| Group attribute           | Runs on |
|---------------------------|---------|
| `#[Group('not-core-12')]` | v13     |
| `#[Group('not-core-13')]` | v12     |

The group name is bound to the core versions of the branch, not to a role. On
`main`, where the supported versions are v13 and v14, `not-core-13` means the
opposite of what it means here — a group attribute must never be carried across
a backport unchanged.

Every occurrence on this branch is on a **single test method**, in two
situations:

* A shared class that differs in one behaviour only —
  `packages/fgtclb/academic-base/Tests/Functional/Environment/EnvironmentBuilderFactoryTest.php:36`
  is `not-core-13` and `:49` is `not-core-12`, the two halves of the same
  assertion about which builder the factory returns.
* A class that exists per core version, below a `Core12/` or `Core13/` folder:
  `packages/fgtclb/academic-base/Tests/Functional/Core12/Environment/StateManagerTest.php`
  and its `Core13/` counterpart. **The folder alone excludes nothing** — the
  PHPUnit configurations glob `Tests/Functional/` recursively, so both classes
  are collected on both core versions and every test method in them carries the
  group. Only PHPStan skips the folders, through its `excludePaths`.

No class-level `#[Group]` is in use today, and there is no `Tests/Unit/Core12/`
or `Tests/Unit/Core13/` folder.
...
(`Build/Scripts/runTests.sh:616-628`, rendering in `executeRstRendering()` at
...
|---------------------|------|--------------------|---------------------------------------|
| `cgl`               | 76   | —                  | PHP 8.1, v12                          |
| `phpstan`           | 118  | —                  | v12/8.1, v13/8.2                      |
| `lint`              | 158  | —                  | PHP 8.1, 8.2, 8.3, 8.4, 8.5           |
| `unit`              | 177  | cgl, phpstan, lint | v12/8.1, v12/8.4, v13/8.2, v13/8.5    |
| `functional-sqlite` | 220  | unit               | the same four pairs                   |
| `functional-dbms`   | 260  | functional-sqlite  | the same four pairs × 4 DBMS, 16 jobs |
| `documentation`     | 307  | —                  | none                                  |
...
(`.github/workflows/ci.yml:260-265`).
...
that need a vendor tree carry a `combo` axis of explicit `{typo3, php}` pairs
instead of a `php-version × typo3` cross product, which would generate
unsupported combinations such as TYPO3 v13 on PHP 8.1. The reasoning is written
out in the workflow header (`.github/workflows/ci.yml:25-39`).

Four sets result, and they have to be changed together:

| Set          | PHP versions                              | Used by              | Why                                            |
|--------------|-------------------------------------------|----------------------|------------------------------------------------|
| all          | 8.1, 8.2, 8.3, 8.4, 8.5                   | `lint`               | A syntax error can be specific to one version. |
| edges of v12 | 8.1, 8.4                                  | `unit`, `functional` | The ends of the range v12 supports.            |
| edges of v13 | 8.2, 8.5                                  | `unit`, `functional` | The ends of the range v13 supports.            |
| lowest       | 8.1 (`cgl`), per core version (`phpstan`) | `cgl`, `phpstan`     | They inspect files, not the running PHP.       |
...
itself: `-b docker` on every step (`.github/workflows/ci.yml:47-55`) and the
...
2. `github.event.workflow_run.pull_requests` is empty for a fork, so the pull
   ...
   (`.github/workflows/ci.yml:315-333`).
