...
Nor would a tmpfs be faster: measured in CI on both branches, a tmpfs for these
directories left the functional jobs exactly as long as the bind mount
(ACE-698).
...
* **`-t` selects configuration, not dependencies.** Running `-t 13 -s phpstan`
  after a `-t 12 -s composerUpdate` analyses v13-flavoured configuration
  against a v12 vendor tree. The result is neither a pass nor a meaningful
  failure.
...
CI. The reasoning is recorded in `.gitignore:16-19`.
...
`core-12/` and `core-13/` at the repository root are ready-to-start TYPO3
...
| `index_words`            | 1037          | 0                  |
| `index_phash`            | 222           | 0                  |
| `cache_pages_tags`       | 509           | 0                  |
| `sys_file_processedfile` | 58            | 0                  |
| **file size**            | **10.93 MB**  | **5.73 MB**        |
...
* the rows of `sys_file`/`sys_file_metadata` whose `storage` is `0` — the
...
* and the autoincrement high water marks `sqlite_sequence` keeps for whatever it
  emptied, so that a snapshot taken from a browsed instance is the same file as
  one taken right after a seed.
...
Backed up var/sqlite/core-13.sqlite -> ../sqlite-databases/core-13.sqlite
    (170 tables, 5.7 MB, 10538 transient rows removed)
```
...
What an instance contains is **described**, not clicked together:
...
plugins read —
profiles with their contracts, the organisational units, the frontend user and
its group. Writing it into an empty instance is one command:
...
comes from `sbuerk/data-factory`, required by both instances
(`core-12/composer.json:18`, `core-13/composer.json:18`). Version `1.0` is the
one that still supports TYPO3 v12 and PHP 8.1; `2.0` requires v13 and PHP 8.2.
The import was run on both versions.

A set is addressed by its identifier, not by a path: the command looks for a
`Configuration/DataFactory/<identifier>/config.yml` in every **active**
extension. `config.yml` names the scenario files the records come from, which is
why the set is a folder rather than a file.

Five things about the set are worth knowing before changing it:
...
- **It declares uids.** `config/sites/academics/config.yaml` points at
  ...
  backend user — which is why the command refuses to run without one.
- **It refuses a non-empty page tree.** A definition declaring uids collides
  rather than adding, so seeding is something you do to a freshly set up
  instance, not to a restored template.
- **One definition serves both core versions.** The backend layouts of
  `bootstrap_package`, the six `academicpersons_*` CTypes and the felogin plugin
  are identical on v12 and v13, so a second file would be two files to keep in
  sync and no difference to express.
- **A plugin FlexForm is a raw XML string in the definition.**
...
**An entity id is at most 27 characters long.** The seeder writes `NEW` plus the
id into `sys_log.NEWid`, which is a `varchar(30)` on TYPO3 v12 — SQLite ignores
the declared length and every other DBMS does not, so an over-long id is a defect
only the SQLite leg would survive.

The TypoScript of the instances comes from the seed too, as one root
...
python3 - <<'EOF'
import yaml, sqlite3
seed = yaml.safe_load(open('packages-dev/dev-site/Configuration/DataFactory/academics-instance/Scenario.yaml'))
...
is what it produced. `core-12/` was rebuilt the same way — the differences are
listed after the sequence.
...
ddev composer instance:seed                        # the page tree, content and records
ddev composer sqlite:backup                        # commit the result
ddev composer sqlite:apply                         # back to normal, clears the marker
```

`setup:begroups:default` is **TYPO3 v13 only** — the command does not exist on
v12, as `vendor/bin/typo3 list` there confirms, so the `core-12` rebuild simply
skips that line.
...
page, so a seed run that puts pages at the tree root — a definition whose
sections are not children of the site root — leaves a set of
...
The rebuilt templates came out at **168 tables** for `core-12` and **171** for
`core-13`, where the hand-maintained ones they replace had **146 each** — the
honest measure of how far a database that is clicked together drifts from what
the code would create today. That is the reason the templates are now produced
by a rebuild and a seed rather than by editing them further.

`core-12` was rebuilt with the same sequence and behaves the same, the two sharp
edges above included. Three things differ, none of them structural: the project
name says `(TYPO3 v12)`, the database file is `var/sqlite/core-12.sqlite`, and
`setup:begroups:default` is skipped. `extension:setup` comes from
`EXT:extensionmanager` on v12 instead of `EXT:core`, but authenticates a backend
user just the same, and the `setup` command has the identical `pdo_sqlite`
branch that ignores the database name.

## See also

- [Development instances](instances.md) — the two DDEV instances, their
  accounts, and what the seed puts on which page.
