...
the content elements, their plugin FlexForms and every record the plugins read —
...
which is `vendor/bin/typo3 theme:seed EXT:academics_dev_site/Configuration/Seeds/Instance.yaml`.
The command comes from `sbuerk/theme-extension-development`, required by both
instances (`core-12/composer.json:18`, `core-13/composer.json:18`). **Only the
seeder is used; the theme itself is not** — both instances are themed with
`bk2k/bootstrap-package`. The package supports TYPO3 v12 and v13, and
`theme:seed` was run on both.

Five things about the definition are worth knowing before changing it:
...
**An identifier is at most 27 characters long.** The seeder writes `NEW` plus
the identifier into `sys_log.NEWid`, which is a `varchar(30)` on TYPO3 v12 —
SQLite ignores the declared length and every other DBMS does not, so an
over-long identifier is a defect only the SQLite leg would survive.

The TypoScript of the instances comes from the definition too, as one root
