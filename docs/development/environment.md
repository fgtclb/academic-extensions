...
instances. **Only the seeder is used; the theme itself is not** — neither site
configuration depends on its site set.

Four things about the definition are worth knowing before changing it:
...
- **One definition serves both core versions.** The backend layouts of
  ...
  are identical on v13 and v14, so a second file would be two files to keep in
...
The package also carries the site set `fgtclb/academics-dev-site`, which is
instance configuration rather than extension code: it sets the page template
name for the two custom page types. `bk2k/bootstrap-package` derives that name
from the backend layout through `case = uppercamelcase`, which lowercases the
whole string first, so the layout `pagets__AcademicProgram` that
