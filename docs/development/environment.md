...
at some 5 MB each would put the better part of a gigabyte into RAM. Nor would a
tmpfs be faster: measured in CI on both branches, a tmpfs for these directories
left the functional jobs exactly as long as the bind mount (ACE-698).
...
with the same group exclusions and the same path or filter given after `--` — and
`Build/Scripts/splitFunctionalTests.php` turns that list into one PHPUnit
configuration per chunk. It then starts itself once per chunk with `-c
...
the number of tests in the list with the number the JUnit logs of the chunks
report, skipped tests included on both sides, and fails the run on any
difference.
...
behind the average in CI, mostly because a few classes of `packages-dev/dev-site`
import the whole development seed. `LegacyDeliveryTest` alone takes 3.6 minutes on
MySQL, which is also the floor no chunk count can go below. Balanced by the
recorded durations, the spread was 0 to 3 %.
...
```bash
gh run download <run-id> -n functional-junit-mysql-8.0-v14-php8.2 -D .Build/junit/mysql
...
Measured on a 32 core, 125 GB workstation, TYPO3 v14, PHP 8.2, with other work
running (ACE-696):

| DBMS      | serial       | `-j 4` | `-j 8` | `-j 12` | `-j 16` |
|-----------|--------------|--------|--------|---------|---------|
| sqlite    | about 10 min | 3:36   | 2:11   | 2:09    | 2:12    |
| mysql 8.0 | about 37 min | —      | 20:15  | —       | 14:22   |

**SQLite stops improving at some 8 chunks.** From there one chunk holds nothing
but `LegacyDeliveryTest`, some 2 minutes on its own, and a class is never split.
More chunks cost nothing but containers either. The real databases keep gaining,
because each chunk waits on its own database far more than on the CPU. The
recorded durations come from CI, where MySQL is not slower than the others; on
a workstation its chunks are therefore less even (11 to 20 minutes at `-j 8`).
