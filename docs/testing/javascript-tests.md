...
import map specifiers and the stubs, and that the request double records and
refuses what it says it does.
...
The cost is one dependency, and it is not free: `jsdom` brings 37 packages on
top of the 158 `Build/node_modules` already held. None of them is distributed
— the repository root is a composer `project`, and nothing below `Build/`
reaches a composer dist or a TER archive.
...
## No library is resolved from outside this repository

Every specifier a frontend module of this extension imports is either one of
this repository's own modules — answered by the rule above — or one of the two
libraries the stubs stand in for. There is no third case: the profile editor's
elements are plain custom elements and depend on no user interface library, so
there is no version that has to be kept in step between the browser and this
suite, and no pin that could silently drift away from what the core ships.
...
geometry. jsdom decodes no images either, so a test about the resolution the
crop is uploaded at says what the natural width of the chosen file is, and the
cropper stub scales the crop box back by it exactly as the library does.
...
1. **No TypeScript that is not erasable.** Node strips type annotations; it does
   ...
   means the custom elements register with a bare `customElements.define()` and
   declare their properties as accessor pairs, rather than with a
   `@customElement` / `@property` decorator — which keeps the esbuild output
   plain as well.
