...
| `field-editing.test.ts`          | Per-field edit, clear, undo and save; only changed fields travel; the validation messages; the visibility switch; the field groups and their preview modes; that opening another editor closes an untouched one, asks about a changed one and saves, discards or keeps it as told, and that every control beside a field refuses and says why while a save is in flight. |
| `full-form-editing.test.ts`      | The other editing state: what entering hides, opens and asks about, that a pencil pressed inside the form does nothing, apply as one request, undo, discard, the keyboard, and autosave suppressed while the form is open.                                                                                                                                               |
...
| `contract-contacts.test.ts`      | The contacts of a contract: their endpoints, their editor, the visibility toggle, and what a save does to the list the element is handed.                                                                                                                                                                                                                                |
...
| `editors.test.ts`                | One editor at a time across the modules: a field pencil and a document row closing, asking about and saving or discarding each other, the row that replaces a changed row, and the dialog itself - its three answers, its dismissal, the focus it returns and the root without a template.                                                                               |
...
| `element-lifecycle.test.ts`      | The base class: that nothing but an element itself writes below it, and the update cycle it hand-writes in place of a framework — batching, deferral, the dropped no-op assignment, and what `updateComplete` settles with when a callback queues another update or throws.                                                                                              |
...
With the three harness self-tests under `Tests/JavaScript/Harness/` that is
**379 cases in 60 suites**, across 27 files. Take the figure from the runner
rather than from a `grep` over the sources: `prototypes.test.ts` generates one
case per prototype, so the 364 cases written there are 379 executed ones.
...
[Frontend assets](../development/frontend-assets.md#libraries-come-from-the-core)
for the rule and for what belongs in `_dependencies.d.ts` instead.
...
- [Frontend assets](../development/frontend-assets.md) — the build, the
  committed artifacts and where the libraries come from.
