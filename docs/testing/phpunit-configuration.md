...
[`Build/Scripts/runTests.sh:664`](../../Build/Scripts/runTests.sh#L664) for the
functional suite and
[`Build/Scripts/runTests.sh:752`](../../Build/Scripts/runTests.sh#L752) and
[`:758`](../../Build/Scripts/runTests.sh#L758) for `unit` and `unitRandom`.
...
The "Value here" column is read off the two files. The "PHPUnit default" column
is PHPUnit's own default for the attribute; confirm it against
`.Build/vendor/phpunit/phpunit/phpunit.xsd` of the installed version if a
...
load-bearing — two functional tests skip themselves on a core version condition,
see [Functional tests](functional-tests.md#version-gated-tests).
...
This branch supports TYPO3 v12 and v13 from one code base, and CI runs every
suite once per core version. A deprecation is version specific by nature: core
raises `E_USER_DEPRECATED` for an API on the newer version while the older one
still considers it current. With `failOnDeprecation="true"` this asymmetry
becomes a red matrix leg — the v13 job fails and the v12 job stays green on
exactly the same commit.

The consequence is that **a deprecation cannot be waited out**. The option is
not relaxed and no suite is narrowed to the older core version; the underlying
code is fixed so that it is deprecation-free on both.

Where an API genuinely has no counterpart on the older version, the way out is
the core-version split — a `Core12/`/`Core13/` folder or a `not-core-*` group —
not a silenced deprecation. See
...
resolves to the global namespace, where no such class is defined. The condition
is therefore always false and the `else` branch always runs. The line comes from
the upstream `typo3/testing-framework` template, so this is inherited, not
introduced here — change it upstream, not in the copy.
