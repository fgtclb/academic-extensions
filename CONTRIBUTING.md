...
This branch supports **TYPO3 v12 and v13**, on PHP 8.1 to 8.5 depending on the
core version — see the support matrix in [`README.md`](README.md). `-t` defaults
to `12`, `-p` to `8.2`.
...
```bash
Build/Scripts/runTests.sh -t 12 -s cgl -n      # coding guidelines, omit "-n" to fix
Build/Scripts/runTests.sh -t 12 -s phpstan     # static analysis, per core version
Build/Scripts/runTests.sh -t 12 -s lintPhp     # PHP linting

Build/Scripts/runTests.sh -t 13 -s composerUpdate
Build/Scripts/runTests.sh -t 13 -s phpstan     # again, against TYPO3 v13
```
...
Build/Scripts/runTests.sh -t 12 -s functional               # SQLite, the default
Build/Scripts/runTests.sh -t 12 -s functional -d postgres   # anything that writes
```
...
`Documentation/Changelog/2.4/` — `Breaking-*.rst`, `Deprecation-*.rst`,
